...
class CheckersNet(nn.Module):
    ...
    def __init__(self, board_size=8, in_channels=5, num_filters=64, action_size=4096):
        """
        ...
        # Policy head
        self.policy_conv = nn.Conv2d(num_filters, 2, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn1(self.conv1(x)))          # shape: (batch_size, num_filters, 8, 8)
        x = F.relu(self.bn2(self.conv2(x)))           # shape: (batch_size, num_filters, 8, 8)
