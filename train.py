...
def self_play_game(net, mcts_simulations=800, temperature=1.0) -> List[Tuple[torch.Tensor, List[float], float]]:
    """
...
def main():
    ...
    # Helper function to wait for user input in the background
    ...
    try:
        ...
        print("Loaded model from latest_iteration.pth")
    except FileNotFoundError:
        print("No checkpoint found. Starting from scratch.")

    optimizer = optim.Adam(net.parameters(), lr=1e-3, weight_decay=1e-4)
    try:
        replay_buffer = torch.load("replay_buffer_latest.pth")
        print("Loaded replay buffer from replay_buffer_latest.pth")
    except FileNotFoundError:
    ...
    iteration_start = 0  # Start from iteration 0 or adjust for resumption

    for iter_idx in range(iteration_start, num_iterations):
        if stop_signal.is_set():
            # Save current progress with specific filenames when "stop" is received
            torch.save(net.state_dict(), f"model_iter_{iter_idx}.pth")
            torch.save(replay_buffer, f"replay_buffer_iter_{iter_idx}.pth")
        ...
        avg_loss, avg_ploss, avg_vloss = train(net, optimizer, replay_buffer, batch_size=32)
        print(f"Iter {iter_idx + 1}/{num_iterations}: loss={avg_loss:.4f}, policy_loss={avg_ploss:.4f}, value_loss={avg_vloss:.4f}")

        # Save the most recent state as "latest_iteration"
        torch.save(net.state_dict(), "latest_iteration.pth")
        torch.save(replay_buffer, "replay_buffer_latest.pth")
        print(f"Checkpoint saved as latest_iteration.pth and replay_buffer_latest.pth")
...
def main():
    ...
    for iter_idx in range(iteration_start, num_iterations):
        ...
        if stop_signal["stop"]:
            ...
            torch.save(net.state_dict(), f"model_iter_{iter_idx}.pth")
            torch.save(list(replay_buffer), f"replay_buffer_iter_{iter_idx}.pth")  # Convert deque to list for saving
            print(f"Training stopped. Saved model as model_iter_{iter_idx}.pth and replay buffer as replay_buffer_iter_{iter_idx}.pth")
            break
        ...
        # Save the most recent state as "latest_iteration"
        torch.save(net.state_dict(), "latest_iteration.pth")
        torch.save(list(replay_buffer), "replay_buffer_latest.pth")  # Convert deque to list for saving
        print(f"Checkpoint saved as latest_iteration.pth and replay_buffer_latest.pth")
